...
def handle_conversations(conversations: List[str],model,eos_token,conversation_fn):
...
def process_file(file, output_dir, from_pct, to_pct,model='qwen', max_workers=10):
    try:
        ...
        output_file_name = f'{output_file_name[:-6]}_labels_{from_pct}_{to_pct}.jsonl'
        ...
        with open(output_file_name, 'w') as output_file:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                futures = []
                for line in lines:
                    future = executor.submit(process_line, line, model, eos_token, conversation_fn)
...
def process_line(line, model, eos_token, conversation_fn):
    line = line.strip()
    if len(line) == 0:
        return None
