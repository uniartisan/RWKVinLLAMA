...
class PipelineParallelModel:
    ...
    def forward(self, input_ids, attention_mask):
        ...
        logger.debug(f'finished moving input_ids and attention_mask to cuda:0')
        ...
        with torch.amp.autocast(device_type='cuda',dtype=torch.bfloat16):
            for gpu_id, stage in enumerate(self.pipeline_stages):
                ...
                del old_musk
                logger.debug(f'finished moving hidden and attention_mask to cuda:{gpu_id}')
                for idx,layer in enumerate(stage):
                    logger.debug(f'processing stage {gpu_id} with layer {idx}')
                    all_hidden_states.append(hidden.detach())
                    layer_outputs = layer(hidden, attention_mask=current_mask,use_cache=False)
                    hidden = layer_outputs[0]
                    logger.debug(f'finished processing stage {gpu_id} with layer {idx}')
                    del layer_outputs
                    logger.debug(f'finished del layer_outputs {gpu_id} with layer {idx}')
                    # 删除不需要的变量
                del current_mask
                logger.debug(f'finished processing stage {gpu_id}')
                # 删除不需要的变量
            del attention_mask
            ...
            logger.debug(f'finished processing final stage')
        del input_ids
    ...
    with cp.cuda.Device(device_id):
        ...
        while True:
            try:
                ...
                for i in range(world_size-1):
                    logger.info(f'Receiving from client {i} with {nccl_ids} {device_id}')
                    comm.recv(recv_buffers[i].data.ptr, batch_size*length, nccl.NCCL_INT64, i+1, stream.ptr)
                logger.info(f'Synchronize stream to wait for all receives to finish {device_id}')
                stream.synchronize()
                logger.info(f'Finished receiving from all clients {device_id}')
                ...
                for i in range(world_size-1):
                    ...
                    logger.debug(f'input_ids : {input_ids} {device_id}')
                    with pipeline_model.forward_lock:
                        logger.debug(f'forward pass for client {i} {device_id}')
                        logits,hidden_states = pipeline_model.forward(input_ids,attention_mask)
                    logger.debug(f'finished forward pass for client {i} {device_id}')
